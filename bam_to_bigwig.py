...
def bam_to_wig(bam_filename, wig_filename, ignore_secondary=False, ignore_qc_fail=False, ignore_optical_pcr_duplicate=False, ignore_supplementary=False):
    ...
    subprocess.check_call(cl, stderr=subprocess.STDOUT)
...
def bam_to_sizes(bam_filename, chr_sizes_filename):
    ...
    with open(chr_sizes_filename, "w") as f:
...
def main(bam_filename, bigwig_filename=None, use_tempfile=False, keep_tempfile=False, ignore_secondary=False, ignore_qc_fail=False, ignore_optical_pcr_duplicate=False, ignore_supplementary=False):
    ...
    bam_to_wig(bam_filename, wig_filename, ignore_secondary, ignore_qc_fail, ignore_optical_pcr_duplicate, ignore_supplementary)
    sys.stdout.write('Generate wig file: %s ...Done\n' % wig_filename)
    ...
    sys.stdout.write('Generate sizes file: %s' % chr_sizes_filename)
    bam_to_sizes(bam_filename, chr_sizes_filename)
    sys.stdout.write(' ...Done\n')
    
    # run wigToBigWig
    sys.stdout.write('Generate bigwig file: %s' % bigwig_filename)
    wig_to_bigwig(wig_filename, bigwig_filename, chr_sizes_filename)
    sys.stdout.write(' ...Done\n')
